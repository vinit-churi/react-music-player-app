  ...
  return (
    ...
    >
      ...
      {currentTrack?.id === song.id ? (
        <div className="group flex-[0_0_max-content] mr-3  h-12 w-12 mx-1 flex justify-center items-center">
          <img
            src={animatedIcon}
            className="flex-[0_0_max-content]  h-8 w-8 object-center object-contain group-hover:hidden cursor-pointer hover:scale-105 "
          />
          {isPlaying ? (
            <BsFillPauseCircleFill
              onClick={() => dispatch(pause())}
              className="hidden group-hover:block object-contain object-center h-8 w-8 cursor-pointer hover:text-cyan-400 transition-all ease-in-out duration-300"
            />
          ) : (
            <AiFillPlayCircle
              onClick={() => dispatch(play())}
              className="hidden group-hover:block object-contain object-center h-8 w-8 cursor-pointer hover:scale-105 hover:text-cyan-400 transition-all ease-in-out duration-300"
            />
          )}
        </div>
      ) : (
